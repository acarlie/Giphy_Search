...
var app = {
  ...
  renderCards (obj) {
    ...
    var cardWrap = $('<div>').addClass('card-wrap').appendTo('#results');
  ...
  formatDate (date) {
    var posted = moment().diff(moment(date, 'YYYY-MM-DD hh-mm-ss'), 'days');
    var giphy = moment().diff(moment('02/01/2013', 'MM/DD/YYYY'), 'days');
    var text = 'Posted ' + moment(date, 'YYYY-MM-DD hh-mm-ss').fromNow();

    return posted < giphy ? text : 'Posted Date N/A';
  },
  search () {
    event.preventDefault();
